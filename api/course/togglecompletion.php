...
include(dirname(dirname(__FILE__)).'/include/config.php'); //My code
...
if($module_id == '1'){ //assignment
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $total_points = $user_points + $assignment_points;
        
        $sql = "UPDATE mdl_user_points SET points='".$total_points."' WHERE user_id='".$c_user_id."' ";
        mysqli_query($con, $sql);
        
    }else{
        $sql = "INSERT INTO mdl_user_points (user_id, points) VALUES ('".$c_user_id."', '".$assignment_points."')";
        mysqli_query($con, $sql);
    }
...
if($module_id == '16'){ //quiz
    ...
    if(mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_assoc($result);
        $user_points = $row['points'];
        $total_points = $user_points + $quiz_point;
        
        $sql = "UPDATE mdl_user_points SET points='".$total_points."' WHERE user_id='".$c_user_id."' ";
        mysqli_query($con, $sql);
        
    }else{
        $sql = "INSERT INTO mdl_user_points (user_id, points) VALUES ('".$c_user_id."', '".$quiz_point."')";
        mysqli_query($con, $sql);
    }
    
    $details = $course_fullname.' - '.$quiz_name;
    $spentTime = date('Y-m-d');
    $sql = "INSERT INTO mdl_user_points_trans (type, detail, amount, userid, spentTime) VALUES ('+', '".$details."', ".$quiz_point.",'".$c_user_id."','".$spentTime."' )";
    mysqli_query($con, $sql);
